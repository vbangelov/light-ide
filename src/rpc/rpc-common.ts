  ...
  handleResponse(message: any): void {
    ...
    if (promiseCallbacks) {
      ...
      if (message.success) {
        ...
        console.warn(`handleResponse: Message id ${message.id} rejected, response: ${message.response}`);
  ...
  async handleRequest(message: any): Promise<void> {
    ...
    if (method) {
      ...
      try {
        ...
        console.error(`handleRequest: Failed processing request ${message.command} id: ${message.id}`, { error: err });
